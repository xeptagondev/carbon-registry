...
export class LetterSustainableDevSupportLetterGen {
  ...
  async generateLetter(
    ...
    const environmentalManagementActHyperlink = this.configService.get(
    ...
    if (ownerNamesList.length > 2) {
      programmeOwner = ownerNamesList
        .slice(0, ownerNamesList.length - 1)
        .join(", ");
    ...
    doc.text(
      "Environmental Management Act, 2007 & Environmental Impact Assessment (EIA) Regulations 2012",
      {
    ...
    doc.text(`Carbon Market Secretariat`, {
      align: "left",
    });
