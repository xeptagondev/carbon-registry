...
describe("LedgerDbService", () => {
  ...
  it("insert record", async () => {
    return service
      .insertRecord({ test_name: "name", val: 700 }, "test_table")
      .then((data) => {});
  });

  it("get record", async () => {
    return service
      .fetchRecords({ test_name: "name" }, "test_table")
      .then((data) => {
  ...
  it("get history", async () => {
    return service
      .fetchHistory({ test_name: "name" }, "test_table")
      .then((data) => {
  ...
  it("update record", async () => {
    const txVal = 9999;
    return service
      .updateRecords({ val: txVal }, { test_name: "name2" }, "test_table")
      .then((data) => {
        return service
          .fetchRecords({ test_name: "name2" }, "test_table")
          .then((data2) => {
